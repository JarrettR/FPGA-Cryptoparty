/*!
   memfifo -- implementation of EZ-USB slave FIFO's (input and output) a FIFO using the DDR3 SDRAM for ZTEX USB-FPGA Modules 2.04
...
/*	    int m=(b[0] & 255) | (b[1] & 255)<<8 | (b[2] & 255)<<16 | (b[3] & 255)<<24;
	    int l=-1;
	    for (int k=4; (k<bb && l<0); k+=4 ) {
		int n=(b[k+0] & 255) | (b[k+1] & 255)<<8 | (b[k+2] & 255)<<16 | (b[k+3] & 255)<<24;
		if ( n!=m+0x03030303 ) l=k;
		m = n;
	    }
	    if (l>=0) {
		System.out.print("Error at "+l+"\n   ");
		for (int k=0; k<32; k++ ) {
		    System.out.print("   " + (b[l+k-4] & 255) );
		    if ( k== 3) System.out.print("\n--> ");
		    else if ( (k & 3) == 3 ) System.out.print("\n   ");
		}
		merrors = 1;
	    }  */
	    
/*	    for ( int l=0; l<1088; l+=1) {
		System.out.print("   " + (b[l] & 255) );
		if ( (l & 15) == 15 ) System.out.println();
		if ( (l & 255) == 255 ) System.out.println();
	    }   */
...
// upload the firmware if necessary
	    if ( force || ! ztex.valid() || ! ztex.dev().productString().equals("memfifo for UFM 2.01") ) {
...
// upload the bitstream if necessary
		...
		System.out.println("FPGA configuration time: " + ztex.configureFpga( "fpga/memfifo.bit" , force, -1 ) + " ms");
	    } 
...
// Mode 0: write+read mode
	    ...
	    t0 = new Date().getTime();
	    ztex.verify(reader, 500, 0);
	    System.out.println("Read data rate: " + Math.round(reader.bufSize*500.0/((new Date().getTime()-t0)*100.0)*0.1) + " MByte/s");

	    System.out.println("USB write + read mode: 5 MByte/s read rate");
	    ztex.verify(reader, 500, 5000);
